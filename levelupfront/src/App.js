import React, { useState } from 'react';
import './App.css';
import CardsHolder from './Components/UI_CardsHolder';
import TopNav from "./Components/UI_TopNav";
import Sidebar from "./Components/UI_Sidebar";
import SettingsPage from "./Components/UI_Settings"; // importa esto

import { Box } from "@mui/material";

function App() {
  const [activeView, setActiveView] = useState("cards"); // "cards" o "settings"

  return (
    <div className="App">
      <Box display="flex" flexDirection="column" height="100vh">
        <TopNav onChangeView={setActiveView} />
        <Box display="flex" flexGrow={1}>
          <Sidebar />
          <Box sx={{ flexGrow: 1, overflow: "hidden" }}>
            {activeView === "cards" && <CardsHolder />}
            {activeView === "settings" && <SettingsPage />}
          </Box>
        </Box>
      </Box>
    </div>
  );
}

export default App;
